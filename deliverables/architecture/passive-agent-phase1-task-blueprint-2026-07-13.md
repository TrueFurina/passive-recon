...
> 硬约束：纯被动红线铁律成立；**零新增第三方依赖**（仅标准库 + 已有依赖）；180/180 测试全绿。
...
| V-P1-17 | 网关超压集成测试通过 + 全量 pytest 180/180 全绿（回归门禁） | 回归 |
...
- 完成 T-P1-1~T-P1-10 后执行 T-P1-11：`pytest` 全量 180/180 绿 + `python scripts/guard_passive.py` 退出码 0。
